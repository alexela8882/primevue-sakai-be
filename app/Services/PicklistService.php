...
class PicklistService{
    ...
    public function getList($listName, $throwErrorIfNotExisting = false, $itemsWithListNameAsKey = false, $checkIfActive = true){
        ...
        if(is_array($listName))
            ...
            $picklists = Picklist::where('name', $listName)->first();
            ...
            foreach($picklists as $picklist) {
                if($checkIfActive)
                    $items[$picklist->name] = makeObject([ 
                              'values' => array_values($picklist->listItems()->where('active', true)->sortBy('order')->all()), 
                              'catSrcListName' => $picklist->catSrcListName 
                         ]);
                else
                    $items[$picklist->name] = makeObject([ 
                                        'values' => array_values($picklist->listItems()->where('_id', '!=', null)->sortBy('order')->all()), 
                                        'catSrcListName' => $picklist->catSrcListName 
                                   ]);
            }
            return $items;
    ...
    public function updateList($listName, $items, $active = true, $report = false) {
        $picklist = Picklist::where('name', $listName)->first();
    ...
    public function deleteLists($name = []) {
        return Picklist::whereIn('name', $name)->delete();
    }
    ...
    public function saveBuilt($listName, $list, $catSrcListName = null) {
        ...
        $pickList = Picklist::firstOrCreate($pickList, $pickList);
